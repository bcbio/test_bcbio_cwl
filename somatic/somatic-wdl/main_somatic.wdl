...
import "jointcall.wdl" as jointcall
import "variantcall.wdl" as variantcall


struct JointvcBatchRec {
 description: String,
 ...
 genome_build: String,
 genome_resources__aliases__human: String,
...
struct AlignmentRec {
 ...
 reference__bwa__indexes: String,
 files: Array[File],
...
workflow main_somatic {
    ...
    Array[String] reference__bwa__indexes
    Array[File] reference__twobit
    ...
    call batch_for_variantcall {
        input: analysis=analysis, 
          ...
          align_bam=postprocess_alignment.align_bam, 
          ...
          reference__snpeff__hg19=reference__snpeff__hg19, 
          description=description
    }


    call summarize_vc {
        input: jointvc_rec=jointcall.jointvc_rec
    }
    ...
    call qc_to_rec {
        input: align_bam=postprocess_alignment.align_bam, 
          analysis=analysis, 
          reference__fasta__base=reference__fasta__base, 
          genome_build=genome_build, 
          config__algorithm__coverage_interval=postprocess_alignment.config__algorithm__coverage_interval, 
    ...
    scatter (jointvc_batch_rec_local in batch_for_jointvc.jointvc_batch_rec) {
  
      call jointcall.jointcall {
          input: jointvc_batch_rec=jointvc_batch_rec_local
      }
   ...
   output {
     Array[File] align_bam = postprocess_alignment.align_bam
     Array[File] summary__multiqc = multiqc_summary.summary__multiqc
     Array[File] validate__grading_summary = summarize_vc.validate__grading_summary
     Array[Array[File]] variants__calls = summarize_vc.variants__calls
     Array[Array[File]] variants__gvcf = summarize_vc.variants__gvcf
   }

}

task batch_for_jointvc {
    Array[Array[VcRec]] vc_rec

    command {
        bcbio_nextgen.py runfn batch_for_jointvc cwl \
    ...
    output {
        Array[Array[JointvcBatchRec]] jointvc_batch_rec = read_struct('wdl.output.jointvc_batch_rec.txt')
    }
...
task batch_for_variantcall {
    ...
    command {
        ...
        ${sep=';;' genome_resources__aliases__ensembl} \
        ${sep=';;' genome_resources__aliases__human} \
        ${sep=';;' genome_resources__aliases__snpeff} \
        ${sep=';;' reference__snpeff__hg19} \
        ${sep=';;' description}
    }
...
task summarize_vc {
    Array[Array[JointvcRec]] jointvc_rec

    command {
        bcbio_nextgen.py runfn summarize_vc cwl \
        sentinel_runtime=cores,1,ram,2048MB \
        ...
        ${write_struct(jointvc_rec)}
    }

    output {
        Array[Array[File]] variants__calls = read_tsv('wdl.output.variants__calls.txt')
        Array[Array[File]] variants__gvcf = read_tsv('wdl.output.variants__gvcf.txt')
